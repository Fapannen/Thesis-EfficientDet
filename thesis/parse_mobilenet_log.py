...
def load_outputs(outputs, num_detections=100):
	...
	for i in range(0, 100):	
		...
		box = [float(item) for item in output.replace(',', '').split()]
		boxes.append(box)

	classes = []
	for i in range(100, 200):
		output = outputs[i][2:-2]
		class_ = [int(output.split('.')[0])]
	...
	for inference in inferenceEntries:
		...
		for i in range(10):
			temp = {
				...
				'bbox'        : [inference.boxes[i][1] * model * (imgwidth / model),
								 inference.boxes[i][0] * model * (imgheight / model),
								 (inference.boxes[i][3] - inference.boxes[i][1]) * model * (imgwidth / model),
								 (inference.boxes[i][2] - inference.boxes[i][0]) * model * (imgheight / model)],
				'score'       : float(inference.confidences[i][0])
			}
...
class InferenceEntry:
	def __init__(self, content):
		...
		self.boxes, self.classes, self.confidences = load_outputs(inference_log[3:-1])
		self.num_detections = int(inference_log[2].split(': ')[1])
